import type { User, Project, ProjectVersion, ApiResponse } from './apiService';
...
export const mockData: Record<string, any> = {
  ...
  getProjectById: {
    ...
    data: {
      ...
      } as Project & { versions: ProjectVersion[] }
    }
  } as ApiResponse,
...
// 模拟SSE流式数据
export const mockSSEData: Record<string, Array<{ type: string; data: any }>> = {
  generateCode: [
  ...
  const sendNext = (): void => {
    ...
    setTimeout(() => {
      try {
        ...
        onError('模拟数据解析错误: ' + (error as Error).message);
      }
